function [X,error_test,time_TNN] = lrtr_Gaussian_tnn(A,b,X_star,Xsize,opts)
...
tcount=toc;
threshold=opts.error;
time_TNN(1)=0;
time_TNN(2)=tcount;
error_test(1)=norm(X_star-X,'fro')/norm(X_star,'fro');
error_test(2)=norm(X_star-X,'fro')/norm(X_star,'fro');
for iter = 1 : max_iter
    t1=tic;
    Xk = X;
    Zk = Z;
    ...
    error_test(iter+2)=norm(X_star-X,'fro')/norm(X_star,'fro');
%     if DEBUG
